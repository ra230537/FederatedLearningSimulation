# plot_accuracy_enhanced.py - Gráficos aprimorados para análise de acurácia
...
#   3) Boxplot por faixas de updates para comparação estatística
...
def plot_smoothed_overlay(data, window, is_non_iid):
    ...
    for label, entries in sorted(data.items(), key=lambda x: int(x[0])):
        ...
        ax.plot(x_axis, smoothed, label=f'{label}% (média móvel, w={window})',
                linewidth=2, color=color)
        # Dados originais com transparência
        ax.plot(x_axis, acc, alpha=0.15, linewidth=0.5, color=color)

    ax.set_xlabel('Número de atualizações (updates)', fontsize=12)
...
def plot_individual_bands(data, window, is_non_iid):
    ...
    labels = sorted(data.keys(), key=lambda x: int(x))
    ...
    for ax, label in zip(axes, labels):
        ...
        ax.set_title(f'{label}% dos clientes', fontsize=12)
...
def plot_boxplot_by_range(data, is_non_iid, n_bins=8):
    """Gráfico 3: Boxplot por faixas de updates para comparação estatística."""
    labels = sorted(data.keys(), key=lambda x: int(x))

    # Achar o maior número de updates entre todas as séries
    max_updates = max(len(data[l]) for l in labels)
    bin_edges = np.linspace(0, max_updates, n_bins + 1, dtype=int)
    ...
    for ax, label in zip(axes, labels):
        ...
        ax.set_title(f'{label}% dos clientes', fontsize=12)
        ax.set_xlabel('Faixa de updates', fontsize=11)
        ax.tick_params(axis='x', rotation=45)
        ax.grid(True, alpha=0.3, axis='y')

    axes[0].set_ylabel('Acurácia do modelo', fontsize=12)
    fig.suptitle('Distribuição da acurácia por faixa de updates', fontsize=13, y=1.02)
    fig.tight_layout()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Gráficos aprimorados de acurácia')
    ...
    print('\n=== Gráfico 3: Boxplot por faixa de updates ===')
