...
Sensibilidade $\rho$                   & 0 / 0{,}075 / 0{,}5 (ablação) & —             \\
...
$\gamma_t = \alpha_0 \cdot \delta^{v} \cdot (1 + \rho\, s)^{-1}$.
As demais variáveis foram mantidas fixas nos valores padrão:
$\alpha_0 = 0{,}8$, $\delta = 0{,}999$, $\rho = 0{,}075$.

\subsection{Impacto do Fator Base $\alpha_0$}

\begin{table}[H]
\centering
\caption{Impacto de $\alpha_0$ na acurácia. Fixos: $\delta=0{,}999$, $\rho=0{,}075$.}
\label{tab:alpha}
\renewcommand{\arraystretch}{1.25}
\resizebox{\textwidth}{!}{%
...
\begin{table}[H]
\centering
\caption{Impacto de $\delta$ na acurácia. Fixos: $\alpha_0=0{,}8$, $\rho=0{,}075$.}
\label{tab:beta}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{ccccc}
\toprule
...
— o orçamento de aprendizado se esgota cedo demais com decaimento agressivo.
...
\subsection{Impacto da Sensibilidade ao \textit{Staleness} $\rho$}

\begin{table}[H]
\centering
\caption{Impacto de $\rho$ na acurácia. Fixos: $\alpha_0=0{,}8$, $\delta=0{,}999$.}
\label{tab:gamma}
\renewcommand{\arraystretch}{1.25}
\resizebox{\textwidth}{!}{%
\begin{tabular}{cccccc}
\toprule
$\rho$ & IID máx. & IID avg\_last10 & IID tail\_std & Non-IID máx. & Non-IID avg\_last10 \\
\midrule
...
Sem penalização ($\rho = 0$), o modelo colapsa para $\approx 16\%$ em Non-IID —
\textbf{falha total}. A penalização por \textit{staleness} é estruturalmente necessária
...
\begin{enumerate}
    ...
    cenários. Recomendação: $\delta \geq 0{,}999$.

    \item \textbf{$\rho$ é dispensável em IID, mas indispensável em Non-IID}:
    sem penalização por \textit{staleness}, o sistema simplesmente não funciona em
    cenários heterogêneos — este resultado sugere que $\rho$ é um pré-requisito
...
$\rho = 0{,}075$) foram aplicados sem ajuste fino a quatro datasets distintos,
...
naturalmente desbalanceados por região, reforça a importância de $\rho$ adaptativo
...
\begin{enumerate}
    ...
    tendem a possuir dados sub-representados.
